<?php
$title = "Reports on National Park Visits";
?>
...
<?php
	$park_n = $parks->count();
	$visits_n = $park_visits->sum('number');
	
	$point_a = $park_visits->mode('nationality');
	$point_d = $park_visits->mode('fee_type');
	$a = $point_a[0];
	$ta = $point_d[0];
?>
   ...
   <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
          <div class="mr-2">
            <p><small>
			Total Number of Parks <span data-feather="users"></span>: <?php echo e($park_n); ?><br>
            Total Number of Visits <span data-feather="users"></span>: <?php echo e($visits_n); ?><br>
...
<!-- begin reports -->
  ...
  <div class="col-sm-12">
  <h4 class="mb-3">Reports on National Park Visits</h4>
  </div> 
	  ...
	  <button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#nationality_c" aria-expanded="false" aria-controls="collapseExample">
		...
		<div class="collapse" id="nationality_c">
		  ...
		  <form class="needs-validation" novalidate  method="POST" action="generate">
				...
				<input type="text" class="form-control" id="type" name="type" value="Park Visits" hidden>
  ...
  <div class="col-sm-4">
    <div class="card">
      <div class="card-body">
        <h5 class="card-title">Report by Park</h5>
        <p class="card-text">Click the button below and select a park.</p>
        
	<p> 
	  <button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#park_c" aria-expanded="false" aria-controls="collapseExample">
		Select Park
	  </button>
	</p>
		<div class="collapse" id="park_c">
		  <div class="card card-body">
		<!-- Begin Form -->
		  <form class="needs-validation" novalidate  method="POST" action="generate">
								<?php echo csrf_field(); ?>
				<!-- -->
				<input type="text" class="form-control" id="report" name="report" value="park_name" hidden>
				<input type="text" class="form-control" id="type" name="type" value="Park Visits" hidden>
				
			<div class="mb-3">
            <label for="category_c">Park Name</label>
            <select class="form-control" id="contintent"  name="filter"  value="" required>
					<option>Choose Park</option>
					<?php $__currentLoopData = $park_visits; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
					<option value="<?php echo e($p->park_name); ?>" ><?php echo e($p->park_name); ?></option>
					<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
					</select>					 
            <div class="invalid-feedback">
              Valid first name is required.
		  ...
		  <form class="needs-validation" novalidate  method="POST" action="generate">
           ...
           <select class="form-control" id="contintent"  name="filter"  value="" required>
					<option>Choose Fee Type</option>
					 <option value="Citizens (ZK)" >Domestic/Citizens (ZK)</option>
					  <option value="Residents/SADC Nationals (USD)"  >Residents/SADC Nationals (USD)</option>
					  <option value="International (USD)"  >International (USD)</option>
					  <option value="Self Drives (Residents/Non-Residents) (USD)"  >Self Drives (Residents/Non-Residents) (USD)</option>
	...
	<!-- Begin Form -->
		  <form class="needs-validation" novalidate  method="POST" action="generate">
            ...
            <input type="text" class="form-control" list="year" id="years" name="filter" placeholder="Year" value="" required>
					...
					<?php $__currentLoopData = $park_visits; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ay): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.header', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\wamp64\www\Tstats\resources\views/Reports/by_parks.blade.php ENDPATH**/ ?>